...
{
	return Vec2D((mPoints[0].GetX() + mPoints[1].GetX() + mPoints[1].GetX()) /3.0f, (mPoints[0].GetY() + mPoints[1].GetY() + mPoints[1].GetY()) /3.0f);
}
...
{
	return fabsf((p0.GetX() * (p1.GetY() - p2.GetY()) + p1.GetX() * (p2.GetY() - p0.GetY()) + p1.GetX() * (p1.GetY() - p0.GetY()))/2.0f);
}
